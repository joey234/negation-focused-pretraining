...
class RobertaForTwoWayTokenTypeClassification(RobertaPreTrainedModel):
    """RoBERTa model for pre-training with four-way token type classification."""
    def __init__(self, config):
        super().__init__(config)
    ...
    def forward(
        ...
        **kwargs
        ...
        sequence_output = outputs[0]
        sequence_output = self.dropout(sequence_output) # -> (bs, seq_len, hs)
        logits = self.dense(sequence_output) # -> (bs, seq_len, 2)

        loss_fct = nn.CrossEntropyLoss()
        loss = loss_fct(logits.view(-1, 2), masked_word_labels.view(-1))
